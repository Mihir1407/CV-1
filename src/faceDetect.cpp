...
int detectFaces(cv::Mat &grey, std::vector<cv::Rect> &faces)
{
  ...
  {
    std::cout << "Loading Haar cascade file from: " << face_cascade_file << std::endl;
    if (!face_cascade.load(face_cascade_file))
    {
      std::cerr << "Unable to load face cascade file from: " << face_cascade_file << std::endl;
      return -1; // Return error code if file can't be loaded
    }
    else
    {
      std::cout << "Haar cascade file loaded successfully." << std::endl;
    }
  }
